...
import { useAllTargets, useCreateTarget, useDeleteTarget } from '@/hooks/useTargets'
...
export default function AdminTargets() {
  ...
  const { data: targets = [], isLoading: targetsLoading, isError } = useAllTargets()

  const mrs = useMemo(() => users.filter(u => u.role === 'mr' && u.is_active), [users])
      ...
      <div className="space-y-4">
        <div className="rounded-xl bg-card p-4 shadow-sm space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Input type="number" min={1} value={targetQty} onChange={e => setTargetQty(e.target.value)} className="touch-target rounded-lg" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label className="text-xs">Start Date</Label>
              <Input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="touch-target rounded-lg" />
            </div>
            <div className="space-y-2">
              <Label className="text-xs">End Date</Label>
              <Input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="touch-target rounded-lg" />
            </div>
          </div>

          <div className="flex gap-2">
            {editingId && (
              <Button type="button" variant="outline" className="flex-1 touch-target rounded-lg" onClick={resetForm}>
                Cancel Edit
              </Button>
            )}
            <Button
