-- Direct INSERT into mr_sub_area_access can still return 403 when RLS policies on this table
-- reference public.users: if users has its own RLS, the policy subquery may see zero rows.
-- This SECURITY DEFINER RPC validates the caller in one place and inserts safely.
...
-- Ensure API role can attempt table ops when policies allow (some projects revoke defaults).
